...
Raw Excel Data
      ...
      ▼
┌─────────────────────┐
│   Power Query (ETL) │  ← Load, clean, reshape, normalize
└─────────────────────┘
      │
      ▼
┌─────────────────────┐
...
└─────────────────────┘
      ...
      ▼
┌─────────────────────┐
│   Dashboard Design  │  ← 6 themed pages, slicers, drill-throughs
└─────────────────────┘
      │
      ▼
  Business Insights
...
Total Sales = SUM(Fact_Orders[Revenue])
Total Profit = SUM(Fact_Orders[Revenue]) - SUM(Fact_Orders[Total_Cost])
Profit Margin % = DIVIDE([Total Profit], [Total Sales], 0)

-- Inventory Efficiency
Stock Turnover Ratio = DIVIDE([Total Sales], AVERAGE(Inventory[Stock_Value]), 0)
Days Inventory Outstanding = DIVIDE(365, [Stock Turnover Ratio])
Overstock Items = COUNTROWS(FILTER(Inventory, Inventory[Stock_Qty] > Inventory[Reorder_Level] * 2))

-- Customer Metrics
Return Rate % = DIVIDE(SUM(Fact_Orders[Returns]), SUM(Fact_Orders[Quantity_Sold]), 0)
Repeat Purchase Rate = DIVIDE([Returning Customers], [Total Unique Customers], 0)
Customer Lifetime Value = DIVIDE([Total Sales], [Total Unique Customers], 0)

-- Logistics
On-Time Delivery Rate = DIVIDE([Orders Delivered On Time], [Total Orders], 0)
Avg Delivery Days = AVERAGE(Logistics[Delivery_Days])
